 ...
 */
configmanagerServices.factory('Session',['$window',
	function($window) {
		var JWT_TOKEN = "_hb.twj";
		var USER = "_hb.user";
		var BINDER = "_hb.configer";
		...
		return {
			...
			setConfiger: function(configer) {
				setValue(BINDER, configer, false);
			},
			getConfiger: function() {
				return getValue(BINDER);
			},
			clearConfiger: function() {
				clearValue(BINDER);
			},
			clearAll: function() {
				clearAll();
