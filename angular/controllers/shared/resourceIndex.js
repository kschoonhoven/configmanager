 ...
 */
configmanagerControllers.controller('ResourceIndexCtrl', ['$scope', 'ConfigerTabs', 'Configer', 'Errors',
	function($scope, ConfigerTabs, Configer, Errors){
		var expanded = false;
		var loading = false;
		...
		$scope.status = {};
		$scope.data = {};
		$scope.data.currentConfigerId = Configer.getCurrent() == undefined ? undefined : Configer.getCurrent().id;
		...
		$scope.$on('configer.selected', function() {
			$scope.data.currentConfigerId = Configer.getCurrent().id;
		});
